...
abstract class AbstractJacksonLayout extends AbstractStringLayout {
	...
	interface ObjectWriter {
		void toJson(Object event, Writer writer) throws IOException;
	}
...
//		if (event instanceof Message || additionalFields.length > 0) {
			// see LogEventJsonMixIn
			String loggerName = event.getLoggerName();
			ReadOnlyStringMap cd = event.getContextData();
			Map wrapped = X.mapx(
...
//					"thrown",event.getThrown(),
			...
			if (additionalFields.length > 0) {
				...
				wrapped.put("additionalFields",additionalFieldsMap);
			}
			return wrapped;
//		} else {
//			return event;
//		}
	...
	public StackTraceElement calcSyslogLocation() {
		String[] cns = new String[] {
				Logging.Print.class.getName(),
				OutputStreamWriter.class.getName(),
				PrintStream.class.getName(),
		};
		// LOG4J2-1029 new Throwable().getStackTrace is faster than Thread.currentThread().getStackTrace().
		final StackTraceElement[] stackTrace = new Throwable().getStackTrace();
		for (int c=0, i=0; i<stackTrace.length; i++) {
			String cn = stackTrace[i].getClassName();
			if (cn.startsWith(cns[c])) {
				for (i++ ; i<stackTrace.length; i++)
					if (!stackTrace[i].getClassName().startsWith(cns[c]))
						break;
				c++;
			}
			if (c==cns.length)
	...
	public void toSerializable(final LogEvent event, final Writer writer) throws IOException {
		objectWriter.toJson(wrapLogEvent(convertMutableToLog4jEvent(event)),writer);
		writer.write(eol);
